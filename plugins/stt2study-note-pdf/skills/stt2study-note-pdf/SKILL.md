...
> `figures/`의 관련 원본 보조자료가 있으면 경로·용도를 노트의 도해 판정에 함께 적어라.
...
> 사용자에게 먼저 알린다.
> PNG/JPG 등 `figures/`의 원본 보조자료는 독립 원본으로도 사용할 수 있다. 이때도
> 반드시 `<figure><img ...></figure>` 안에 넣고, 전체 비율·네 변·하단 여백을 보존한다.
> 매우 큰 이미지는 전체 비율을 유지한 축소 사본만 허용하며, 크롭·분할·핵심 축의
> 재그림은 하지 않는다.
>
